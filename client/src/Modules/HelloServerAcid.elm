module Modules.HelloServerAcid exposing (main)
...
import CodeGen.HelloServerAcid as HSA

type Msg = SendHttpRequest
         | DataReceived (Result Http.Error (HSA.ServerState))
         | UpdateState
         | IncrementCtr

type alias Model = { serverState : HSA.ServerState
                   , localState : String
                   , errorMsg : Maybe String }

initModel : Model
initModel = { serverState = HSA.ServerState "" 0
            , localState = ""
            , errorMsg = Nothing }
...
getModel : Cmd Msg
getModel = HSA.getRequest DataReceived

setCounter : Int -> Cmd Msg
setCounter n = HSA.putSetCounterByCounter n DataReceived
